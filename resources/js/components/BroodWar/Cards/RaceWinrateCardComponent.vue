<template>
    <v-card v-if="racesWinrate" tile class="mt-10 custom-elevation-2" :width="$vuetify.breakpoint.xs ? '100%' : '250'" height="450">
        <v-toolbar
            dark
            dense
            color="teal"
        ...
        <v-container>
            <v-list subheader dense>
                <v-list-item v-for="raceData in racesWinrate" :key="raceData.map">
                    <v-list-item-content>
                ...
                <v-list-item v-for="(raceData, index) in racesWinrate" :key="`${raceData.map}-${index}`">
                    <v-list-item-content>
...
export default {
    ...
    computed: {
        racesWinrate() {
            return this.$store.getters['racesWinrate/racesWinrate'];
        },
    },
    methods: {
        getTextColor(value) {
