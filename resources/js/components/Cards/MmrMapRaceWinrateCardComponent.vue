    ...
    <v-card v-if="mmrMapRaceWinrate" tile class="mt-10 custom-elevation-2" width="100%">
        ...
        <v-container>
            <v-list subheader dense>
                <v-row>
                    <v-col>
                        <v-row v-for="(rankData, index) in ranks" :key="`${rankData}-${index}`">
...
export default {
    ...
    watch: {
        mmrMapRaceWinrate(value) {
            if (value) {
                let valueClone = {...value};
                let allMatchups = [];
                Object.keys(valueClone).map(i => {
                    Object.keys(value[i]).map(j => {
                ...
                Object.keys(value).map(i => {
                    let mapsWithValues = Object.keys(value[i]).map(j => {
                        allMatchups.map(l => {
                            if (value[i][j].some(item => item.matchupShorthand == l) == false) {
                                value[i][j].push({
                                    matchupShorthand: l,
                                    winPercentage: '-',
                                })
                    ...
                    this.ranks.push({rank: i, maps: mapsWithValues});
                })
            }
            this.mmrMapRaceWinrate = value;
        },
    },
    methods: {
        getTextColor(value) {
